...
/// RomCleanupSettingsValidator (56.5%), RunResultValidator (58.8%),
...
public class CoverageGapBatchTests
{
    ...
    public void Validate_ValidSettings_NoErrors()
    {
        ...
        var errors = RomCleanupSettingsValidator.Validate(settings);
        Assert.Empty(errors);
    }
    ...
    public void SafetyValidator_IsProtectedSystemPath_TempDir_False()
    {
        ...
        var temp = Path.Combine(Path.GetTempPath(), "romcleanup_test");
    ...
    {
        var tempPath = Path.Combine(Path.GetTempPath(), "romcleanup_safe_test", "report.csv");
