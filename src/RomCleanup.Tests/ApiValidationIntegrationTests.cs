...
public sealed class ApiValidationIntegrationTests
{
    ...
    public async Task CollectionsMergeRollback_NonexistentAuditFile_Returns404()
    {
        ...
        var tempPath = Path.Combine(Path.GetTempPath(), "RomCleanup_ApiTest_" + Guid.NewGuid().ToString("N"), "audit.csv");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "RomCleanup_ApiValidTest_" + Guid.NewGuid().ToString("N"));
