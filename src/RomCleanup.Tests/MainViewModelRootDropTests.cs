...
public sealed class MainViewModelRootDropTests
{
    ...
    {
        var rootA = Path.Combine(Path.GetTempPath(), "RomCleanup_RootDrop_" + Guid.NewGuid().ToString("N"));
        var rootB = Path.Combine(Path.GetTempPath(), "RomCleanup_RootDrop_" + Guid.NewGuid().ToString("N"));
        var filePath = Path.Combine(Path.GetTempPath(), "RomCleanup_RootDrop_" + Guid.NewGuid().ToString("N") + ".txt");
    ...
    {
        var missingPath = Path.Combine(Path.GetTempPath(), "RomCleanup_RootDrop_" + Guid.NewGuid().ToString("N"));
        var filePath = Path.Combine(Path.GetTempPath(), "RomCleanup_RootDrop_" + Guid.NewGuid().ToString("N") + ".txt");
        File.WriteAllText(filePath, "not a folder");
