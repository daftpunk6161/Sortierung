...
public sealed class UncPathTests
{
    ...
    public void ResolveChildPathWithinRoot_UncChild_InsideRoot_Succeeds()
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "RomCleanup_UNC_" + Guid.NewGuid().ToString("N")[..8]);
    ...
    {
        var fs = new FileSystemAdapter();
        var tempDir = Path.Combine(Path.GetTempPath(), "RomCleanup_TRV_" + Guid.NewGuid().ToString("N")[..8]);
        Directory.CreateDirectory(tempDir);
        try
        {
