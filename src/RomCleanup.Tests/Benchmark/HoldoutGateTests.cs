...
public sealed class HoldoutGateTests : IClassFixture<BenchmarkFixture>
{
    ...
    public void Holdout_DriftWithinTolerance()
    {
        ...
        if (!enforce)
        {
            _output.WriteLine("Holdout drift gate is informational only. Set ROMCLEANUP_ENFORCE_QUALITY_GATES=true to enforce.");
            return;
        }
    ...
    public void Holdout_DetectionRate_AboveMinimum()
    {
        ...
        if (!enforce)
        {
            _output.WriteLine("Holdout detection-rate gate is informational only. Set ROMCLEANUP_ENFORCE_QUALITY_GATES=true to enforce.");
            return;
        }
