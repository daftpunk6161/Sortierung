...
public sealed class RomHeaderInfoContractsIssue9RedTests
{
    ...
    public void RomHeaderInfo_ShouldExistInContractsAssembly_Issue9A43()
    {
        ...
        var contracts = AppDomain.CurrentDomain
            .GetAssemblies()
            .FirstOrDefault(a => string.Equals(a.GetName().Name, "RomCleanup.Contracts", StringComparison.Ordinal));

        // Act
        var type = contracts?.GetType("RomCleanup.Contracts.Models.RomHeaderInfo", throwOnError: false);
    ...
    public void RomHeaderInfo_ShouldHaveSingleDefinitionAcrossLoadedAssemblies_Issue9A43()
    {
        ...
        Assert.Equal("RomCleanup.Contracts.Models.RomHeaderInfo", candidates[0]);
    }
