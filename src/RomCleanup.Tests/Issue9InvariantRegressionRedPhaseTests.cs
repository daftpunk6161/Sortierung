...
public sealed class Issue9InvariantRegressionRedPhaseTests : IDisposable
{
    ...
    {
        var goldenFile = Path.Combine(GetRepoRoot(), "src", "RomCleanup.Tests", "Snapshots", "inv02-runresult-golden.json");
    ...
    public void Should_HaveSnapshotSuiteFiles_When_M02SnapshotsReady_Issue9_M02()
    {
        ...
        {
            var path = Path.Combine(GetRepoRoot(), "src", "RomCleanup.Tests", "Snapshots", snapshot);
    ...
    public void Should_RequireNoDirectFileIOInCoreSetParsers_When_M06Done_Issue9_M06()
    {
        ...
        {
            Path.Combine(GetRepoRoot(), "src", "RomCleanup.Core", "SetParsing", "CueSetParser.cs"),
            Path.Combine(GetRepoRoot(), "src", "RomCleanup.Core", "SetParsing", "GdiSetParser.cs"),
            Path.Combine(GetRepoRoot(), "src", "RomCleanup.Core", "SetParsing", "CcdSetParser.cs"),
            Path.Combine(GetRepoRoot(), "src", "RomCleanup.Core", "SetParsing", "M3uPlaylistParser.cs"),
            Path.Combine(GetRepoRoot(), "src", "RomCleanup.Core", "SetParsing", "MdsSetParser.cs")
        };
    ...
    {
        var testsDir = Path.Combine(GetRepoRoot(), "src", "RomCleanup.Tests");
    ...
    {
        var path = Path.Combine(GetRepoRoot(), "src", "RomCleanup.Tests", "Snapshots", fileName);
