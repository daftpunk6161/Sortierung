...
public sealed class AuditComplianceTests : IDisposable
{
    ...
    /// <summary>
    /// Walk up from test output directory to find the Romulus.UI.Wpf source project directory.
    ...
    private static string FindUiProjectDir([System.Runtime.CompilerServices.CallerFilePath] string? callerPath = null)
    {
        ...
        var candidate = Path.Combine(repoRoot, "src", "Romulus.UI.Wpf");
        if (Directory.Exists(candidate))
            return candidate;

        return Path.Combine("src", "Romulus.UI.Wpf");
    }
