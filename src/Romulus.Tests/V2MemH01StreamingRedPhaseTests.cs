...
public sealed class V2MemH01StreamingRedPhaseTests
{
    ...
    {
        var type = Type.GetType("Romulus.Infrastructure.Orchestration.StreamingScanPipelinePhase, Romulus.Infrastructure");
        Assert.NotNull(type);
    }
    ...
    {
        var type = Type.GetType("Romulus.Contracts.Ports.IAsyncFileScanner, Romulus.Contracts");
        Assert.NotNull(type);
    }
    ...
    {
        var file = Path.Combine(GetRepoRoot(), "src", "Romulus.Infrastructure", "Orchestration", "RunOrchestrator.cs");
