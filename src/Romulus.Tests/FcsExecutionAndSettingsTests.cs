...
public sealed class FcsExecutionAndSettingsTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "Romulus_FX_" + Guid.NewGuid().ToString("N"));
    ...
    private sealed class ConfigurableDialogService : IDialogService
    {
        ...
        public bool ConfirmConversionReview(string title, string summary, IReadOnlyList<Romulus.Contracts.Models.ConversionReviewEntry> entries) => NextConfirm;
