...
public sealed class HardRegressionInvariantTests : IDisposable
{
    ...
    public async Task INV_PAR_02_ApiRunResult_MustMatch_OrchestratorResult()
    {
        ...
        // Run API
        var manager = new Romulus.Api.RunManager(new FileSystemAdapter(), new Romulus.Infrastructure.Audit.AuditCsvStore());
        var apiRun = manager.TryCreate(new Romulus.Api.RunRequest
        {
            Roots = new[] { root },
            Mode = "DryRun",
            PreferRegions = new[] { "US", "EU" }
    ...
    public async Task INV_STA_03_ApiStatus_MustNotBe_Completed_WhenCancelled()
    {
        ...
        var manager = new Romulus.Api.RunManager(new FileSystemAdapter(), new Romulus.Infrastructure.Audit.AuditCsvStore());
        var run = manager.TryCreate(new Romulus.Api.RunRequest
        {
            Roots = new[] { root },
            Mode = "DryRun"
        }, "DryRun");
    ...
    public void INV_AUD_03_ConsoleSorter_MustWrite_AuditRows()
    {
        ...
        var detector = Romulus.Core.Classification.ConsoleDetector.LoadFromJson(consolesJson, new Romulus.Core.Classification.DiscHeaderDetector());
    ...
    public async Task INV_HSC_03_ErrorCount_Parity_Api_Vs_Orchestrator()
    {
        ...
        Assert.Equal(Romulus.Api.RunWaitDisposition.Completed, waitResult.Disposition);
        var completed = manager.Get(run.RunId);
    ...
    public void INV_AUD_05_RollbackTrail_MustContain_ForensicDetails()
    {
        ...
        var fs = new FileSystemAdapter();
        var audit = new Romulus.Infrastructure.Audit.AuditCsvStore(fs);
